...
public class MainMenuController {
	...
	this.tenants = TenantList.getInstance();
	this.income = RentalIncomeReport.getInstance();
	this.expense = ExpensePaymentReport.getInstance();
    ...
    /**
     * Gets the apartment number of the tenant from the console input using a
     * scanner.
     *
    ...
    /**
     * Gets the unique 'lazy' MainMenuController instance. Used instead calling the
     * default constructor.
     *
	...
	System.out.printf("%n%s %s %s %s %s %s %s %s %s %s %s %s %s %n", "AptNo", "Jan", "Feb", "Mar", "Apr", "May",
	...
	String report = this.income.displayRecords();
	...
	System.out.printf("%n%s %s %s %s %n", "Date", "Payee", "Amount", "Category");
	for (int i = 0; i < 30; i++) {
	    System.out.print("-");
	} // end for
	...
	this.tenants.recordTenants();
	this.income.recordIncomePayment();
	this.expense.recordExpensePayment();
